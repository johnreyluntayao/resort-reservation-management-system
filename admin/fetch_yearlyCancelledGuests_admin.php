...
$sql = "SELECT YEAR(d.check_in_date_and_time) AS year, COUNT(*) AS total_guests
        FROM details_and_packages AS d JOIN confirmation as c ON c.package_id = d.package_id
        WHERE YEAR(d.check_in_date_and_time) BETWEEN ? AND ? AND c.status = 'Canceled'
        GROUP BY YEAR(d.check_in_date_and_time)
        ORDER BY year";
